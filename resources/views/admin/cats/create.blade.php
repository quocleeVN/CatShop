...
<div>
    ...
    <div class="bg-white rounded-2xl shadow-sm p-6">
        <h1 class="text-2xl font-bold mb-6">Thêm mèo mới</h1>
        ...
        <form action="{{ route('admin.cats.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" class="w-full rounded-xl border-gray-300" required>
                ...
                <div>
                    ...
                    <select name="breed_id" id="breed_id" class="w-full rounded-xl border-gray-300" required>
                        ...
                        @foreach($breeds as $breed)
                            <option value="{{ $breed->breed_id }}" {{ old('breed_id') == $breed->breed_id ? 'selected' : '' }}>
                                {{ $breed->breed_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="price" id="price" value="{{ old('price') }}" class="w-full rounded-xl border-gray-300" required>
                    @error('price') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="age_in_months" class="block text-sm font-medium mb-1">Tuổi (tháng)</label>
                    <input type="number" name="age_in_months" id="age_in_months" value="{{ old('age_in_months') }}" class="w-full rounded-xl border-gray-300" required>
                    @error('age_in_months') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="color" id="color" value="{{ old('color') }}" class="w-full rounded-xl border-gray-300">
                </div>
                <div>
                    <label for="weight" class="block text-sm font-medium mb-1">Cân nặng (kg)</label>
                    <input type="number" step="0.01" name="weight" id="weight" value="{{ old('weight') }}" class="w-full rounded-xl border-gray-300">
                </div>
                <div>
                    <label for="health_status" class="block text-sm font-medium mb-1">Tình trạng sức khỏe</label>
                    <input type="text" name="health_status" id="health_status" value="{{ old('health_status') }}" class="w-full rounded-xl border-gray-300">
                </div>
                <div>
                    ...
                    <select name="stock_status" id="stock_status" class="w-full rounded-xl border-gray-300" required>
                        <option value="available" {{ old('stock_status') == 'available' ? 'selected' : '' }}>Còn hàng</option>
                        <option value="sold" {{ old('stock_status') == 'sold' ? 'selected' : '' }}>Đã bán</option>
                        <option value="reserved" {{ old('stock_status') == 'reserved' ? 'selected' : '' }}>Đã đặt</option>
                    </select>
                </div>
                <div>
                    <label for="link_image" class="block text-sm font-medium mb-1">Link ảnh</label>
                    <input type="url" name="link_image" id="link_image" value="{{ old('link_image') }}" class="w-full rounded-xl border-gray-300">
                </div>
                <div class="flex items-center mt-8">
                    <input type="checkbox" name="is_vaccinated" id="is_vaccinated" value="1" {{ old('is_vaccinated') ? 'checked' : '' }} class="rounded border-gray-300 text-orange-500">
            ...
            <button type="submit" class="bg-orange-500 text-white px-6 py-3 rounded-xl font-semibold hover:bg-orange-600 transition">Thêm mèo</button>
        </form>
    </div>
</div>
@endsection